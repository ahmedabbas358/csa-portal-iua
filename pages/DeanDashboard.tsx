...
const DeanDashboard: React.FC<DeanDashboardProps> = ({ lang, onLogout, state, security }) => {
  ...
  const generateKey = () => {
    ...
    const finalToken = `CSA-${selectedRole.substring(0,3).toUpperCase()}-${token}`;
  ...
  const handleSecurityUpdate = (e: React.FormEvent) => {
      e.preventDefault();
      // Verify current key
      if (currentKeyInput !== security.config.masterKey) {
          setSecurityMessage({ text: isRtl ? 'الرمز الحالي غير صحيح' : 'Current Key Incorrect', type: 'error' });
          return;
      }
      
      // Update logic
      const updatedConfig = { ...security.config };
      let changed = false;
      ...
      if (newKeyInput) {
          ...
          updatedConfig.masterKey = newKeyInput;
          changed = true;
      }

      if (newAnswer) {
          updatedConfig.securityQuestion = newQuestion;
          updatedConfig.securityAnswer = newAnswer;
          changed = true;
      }

      if (changed) {
          updatedConfig.lastChanged = new Date().toLocaleString();
          security.updateConfig(updatedConfig);
