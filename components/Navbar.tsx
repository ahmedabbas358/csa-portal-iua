...
const Navbar: React.FC<NavbarProps> = ({ lang, setLang, currentPage, setPage, isLoggedIn, settings, isDarkMode, setIsDarkMode }) => {
  ...
  return (
    ...
    >
          ...
          <div className="flex items-center cursor-pointer group" onClick={() => setPage('home')}>
            <div className="flex items-center gap-3">
              ...
              <div className="hidden lg:block">
                <h1 className={`text-lg font-bold leading-tight transition-colors ${showSolidNav ? 'text-gray-800 dark:text-white' : 'text-gray-800 dark:text-white md:text-white md:drop-shadow-md'}`}>
          ...
          <div className={`hidden md:flex items-center p-1.5 rounded-full transition-all duration-300 ${showSolidNav ? 'bg-gray-100/50 dark:bg-slate-800/50 border border-gray-200/50 dark:border-slate-700/50' : 'bg-white/10 backdrop-blur-md border border-white/20 shadow-lg'}`}>
            {navItems.map((item) => (
              <button
                ...
                className={`px-5 py-2 rounded-full text-sm font-bold transition-all duration-300 relative overflow-hidden ${currentPage === item.id
                  ? 'shadow-md transform scale-105'
                  : showSolidNav ? 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200/50 dark:hover:bg-slate-700/50' : 'text-white/90 hover:text-white hover:bg-white/10'
                  }`}
                style={{
                  backgroundColor: currentPage === item.id ? (showSolidNav ? 'var(--primary-color)' : 'white') : 'transparent',
                  color: currentPage === item.id ? (showSolidNav ? 'var(--primary-contrast)' : settings.primaryColor) : undefined
                }}
              >
                {item.label}
              </button>
      ...
      {/* Mobile menu using Portal to escape parent transform/z-index constraints */}
      {isOpen && createPortal(
        <div className="fixed inset-0 z-[9999] bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl animate-fade-in flex flex-col font-sans" dir={isRtl ? 'rtl' : 'ltr'}>
          ...
          <div className="p-6 space-y-2 flex-grow overflow-y-auto">
            {navItems.map((item, idx) => (
              <button
                ...
                className={`block w-full text-left px-6 py-5 rounded-2xl text-xl font-bold transition-all duration-300 transform ${currentPage === item.id
